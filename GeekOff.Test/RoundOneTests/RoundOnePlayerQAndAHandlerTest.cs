...
public class RoundOnePlayerQAndAHandlerTest
{
    ...
    public RoundOnePlayerQAndAHandlerTest()
    {
        _contextGo = Substitute.For<ContextGo>();
        
        _serviceProvider = _services
            .AddMediatR(cfg => cfg.RegisterServicesFromAssemblies(typeof(RoundOnePlayerQAndAHandler).Assembly))
    ...
    [Fact]
    public async Task Handle_RoundOnePlayerQAndAHandler_Questions()
    {
        // Arrange
        _ = _serviceProvider.GetRequiredService<IMediator>();

        var handler = new RoundOnePlayerQAndAHandler.Handler(_contextGo);
        var request = new RoundOnePlayerQAndAHandler.Request(){
            YEvent = "t24"
        };
